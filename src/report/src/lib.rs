//! Report service and corresponding HTTP server that accepts incoming
//! requests from the report client SPA app and static site in order to provide
//! reporting capabilities for the data collected in the ingest crate.
//!
//! `bin/report_server.rs` can be run as a HTTP server and relies on axum
...
//!
//! All ENV variables are prefixed with `SALUS_REPORT_` and use the `conf`
//! crate for getting all configuration. The list of possible settings for
//! this app are as follows:
//! - `SALUS_REPORT_LAYER_COMPRESSION_DEFLATE` - OPTIONAL - values of `true` or `false` to
//!   enable or disable deflate compression. If neither this nor gzip are set,
//!   both default to true.
//! - `SALUS_REPORT_LAYER_COMPRESSION_GZIP` -OPTIONAL - values of `true` or `false` to
//!   enable or disable gzip compression. If neither this nor deflate are set
//!   then both will default to true.
//! - `SALUS_REPORT_LAYER_CORS_MAX_AGE_SECS` - OPTIONAL - integer value accepted
...
//! - `SALUS_REPORT_LAYER_CORS_ORIGINS` - REQUIRED - list of strings that specify
//!   the domains that are allowed to submite CORS requests to this server.
//! - `SALUS_REPORT_LAYER_TIMEOUT_MILLIS` - OPTIONAL - Integer accepted to
//!   specify the timeout for all requests. If no value is provided, default of
//!   30 seconds will be used.
//! - `SALUS_REPORT_LISTENER_IPV4` or `SALUS_REPORT_LISTENER_IPV6` - OPTIONAL -
...
//! - `SALUS_REPORT_LISTENER_PORT` - REQUIRED - Integer value accepted to
//!   specify the port on which this server will listen and respond to requests
//! - `SALUS_REPORT_METRICSDB_DATABASE` - REQUIRED - Name of the metrics
//!   database in the specified Clickhouse instance in which data should be
//!   stored
//! - `SALUS_REPORT_METRICSDB_PASS` - REQUIRED - Password to log into the
//!   associated Clickhouse instance
//! - `SALUS_REPORT_METRICSDB_URL` - REQUIRED - URL for the associated
//!   Clickhouse instance
//! - `SALUS_REPORT_METRICSDB_USER` - REQUIRED - User on Clickhouse instance
//!   that should be used for recording data
//! - `SALUS_REPORT_TRACING` - OPTIONAL - string which must be a valid tracing
